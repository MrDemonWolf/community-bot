...
Community Bot uses [Turborepo](https://turbo.build/repo) for monorepo management with bun workspaces.
...
- `discord-moderation.prisma` — DiscordCase, DiscordCaseNote, DiscordWarnThreshold
- `discord-templates.prisma` — DiscordMessageTemplate
- `discord-scheduled.prisma` — DiscordScheduledMessage
- `discord-custom-commands.prisma` — DiscordCustomCommand
- `discord-logging.prisma` — DiscordLogConfig
- `discord-reports.prisma` — DiscordReport
- `discord-roles.prisma` — DiscordRolePanel, DiscordRoleButton
...
- `listenWithFallback(app, options)` — Starts the server with automatic port fallback on EADDRINUSE, returns `Promise<Server>`

### `@community-bot/api`

Shared tRPC API routers and utilities consumed by the web dashboard. Includes:

- **Core routers:** `auditLog`, `user`, `userManagement`, `setup`
- **Twitch routers:** `botChannel`, `chatCommand`, `regular`, `counter`, `timer`, `spamFilter`, `songRequest`, `playlist`, `giveaway`, `poll`, `queue`, `quote`
- **Discord routers:** `discordGuild`, `discordModeration`, `discordTemplates`, `discordScheduled`, `discordRoles`, `discordCustomCommands`
