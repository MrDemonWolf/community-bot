...
Song request changes      →   song-request:*           →   Other side syncs
Playlist CRUD             →   playlist:*               →   Other side syncs
Giveaway actions          →   giveaway:*               →   Other side syncs
Discord custom cmds CRUD  →   discord:commands-updated →   Discord re-registers cmds
Discord log config change →   discord:log-config-*     →   Discord invalidates cache
Discord mod action        →   discord:case-created     →   —
```
...
| `playlist:created` | `{ playlistId, channelId }` | Web/Twitch | Any |
| `playlist:updated` | `{ playlistId, channelId }` | Web/Twitch | Any |
| `playlist:deleted` | `{ playlistId, channelId }` | Web/Twitch | Any |
| `playlist:activated` | `{ playlistId, channelId }` | Web/Twitch | Any |

Song request events fire when the queue changes or settings are updated. Playlist events fire for CRUD operations and when switching the active backup playlist.

### Giveaways

| Event | Payload | Publisher | Subscriber |
|-------|---------|-----------|------------|
| `giveaway:started` | `{ giveawayId, channelId }` | Web/Twitch | Any |
| `giveaway:ended` | `{ giveawayId, channelId }` | Web/Twitch | Any |
| `giveaway:winner` | `{ giveawayId, channelId }` | Web/Twitch | Any |

Giveaway events fire when a giveaway is started, ended, or a winner is drawn.

### Discord Moderation & Bot Features

| Event | Payload | Publisher | Subscriber |
|-------|---------|-----------|------------|
| `discord:mute` | `{ guildId, muted }` | Web | Discord |
| `discord:case-created` | `{ caseId, guildId }` | Discord | Any |
| `discord:scheduled-send` | `{ scheduledMessageId, guildId }` | Discord | Discord |
| `discord:commands-updated` | `{ guildId }` | Web | Discord |
| `discord:log-config-updated` | `{ guildId }` | Web | Discord |

`discord:case-created` fires when a moderation case is created. `discord:scheduled-send` triggers a scheduled message to be sent. `discord:commands-updated` causes the Discord bot to re-register guild-scoped custom commands. `discord:log-config-updated` invalidates the log config cache.
