...
**Quick Start**: Before configuring env vars, create your [Twitch application](/docs/getting-started/create-twitch-app) and [Discord application](/docs/getting-started/create-discord-app) to get the required credentials. Each `.env.example` file includes inline comments explaining where to find each value.
...
| `CORS_ORIGIN` | string | `"*"` | No | CORS origin for API |
| `WEB_URL` | string | — | No | Web dashboard URL for the `!commands` link |
| `NODE_ENV` | enum | `"production"` | No | Environment: development, production, test |
...
| `TWITCH_APPLICATION_CLIENT_SECRET` | string | — | Yes | Twitch application client secret |
| `REDIS_URL` | string | — | Yes | Redis connection URL (for EventBus pub/sub) |
| `DISCORD_BOT_TOKEN` | string | — | Yes | Discord bot token (for fetching guild channels/roles via REST API) |
| `NEXT_PUBLIC_COMPANY_NAME` | string | `"Community Bot"` | No | Company/brand name used on TOS and Privacy Policy pages |
| `NEXT_PUBLIC_PRIVACY_POLICY_URL` | string | — | No | External Privacy Policy URL. If set, footer links open this instead of the built-in `/privacy` page |
| `NEXT_PUBLIC_TERMS_OF_SERVICE_URL` | string | — | No | External Terms of Service URL. If set, footer links open this instead of the built-in `/terms` page |
...
| `NEXT_PUBLIC_SOCIAL_LINKS` | string | — | No | Comma-separated social URLs (icons auto-detected from hostname — see below) |
...
The **channel name** and **channel URL** on the landing page are derived automatically from the broadcaster's Twitch username (set during setup). No env var is needed.

The **logo** is served from a local asset at `/logo-white-border.png` in `apps/web/public/`. Replace this file to change the logo.

### Supported Social Link Platforms

When you set `NEXT_PUBLIC_SOCIAL_LINKS`, icons are automatically detected from the URL hostname:

| Platform | Detected Hostnames | Icon |
|----------|--------------------|------|
| Twitter / X | `twitter.com`, `x.com` | Twitter icon |
| GitHub | `github.com` | GitHub icon |
| Twitch | `twitch.tv` | Twitch icon |
| YouTube | `youtube.com`, `youtu.be` | YouTube icon |
| Other | Any other URL | Globe icon |

Example:

```bash
NEXT_PUBLIC_SOCIAL_LINKS="https://twitter.com/you,https://github.com/you,https://twitch.tv/you"
```
