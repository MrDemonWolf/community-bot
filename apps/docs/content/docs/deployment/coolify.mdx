...
2. Set the database name to `postgres`
...
```
postgresql://postgres:GENERATED_PASSWORD@PROJECT_UUID-postgres:5432/postgres
```
...
|----------|---------|-----------------|
| `DATABASE_URL` | `postgresql://postgres:pass@host:5432/postgres` | [Step 2](#step-2-postgresql-database) — use the Coolify internal URL |
...
|----------|---------|-------------|
| `NEXT_PUBLIC_COMPANY_NAME` | `MrDemonWolf, Inc.` | Company name on TOS/Privacy pages (default: "Community Bot") |
...
| `NEXT_PUBLIC_PRIVACY_POLICY_URL` | `https://example.com/privacy` | External privacy policy URL (overrides built-in `/privacy` page) |
| `NEXT_PUBLIC_TERMS_OF_SERVICE_URL` | `https://example.com/terms` | External TOS URL (overrides built-in `/terms` page) |
| `NEXT_PUBLIC_SOCIAL_LINKS` | `https://twitter.com/you,https://github.com/you` | Comma-separated social URLs (icons auto-detected) |
...
| Variable | Example | Where to Get It |
|----------|---------|-----------------|
| `DATABASE_URL` | `postgresql://postgres:pass@host:5432/postgres` | [Step 2](#step-2-postgresql-database) |
