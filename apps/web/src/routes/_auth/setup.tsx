...
function SetupWizard() {
  ...
  useEffect(() => {
    if (connected) {
      ...
      invalidate();
      setStep(2);
      navigate({ to: "/setup", search: {}, replace: true });
    }
  ...
  return (
    <div className="container mx-auto flex max-w-xl flex-col gap-6 px-4 py-10">
      ...
      {step === 2 && (
        <TwitchStep data={data} onBack={() => setStep(1)} onNext={() => setStep(3)} />
      )}
      {step === 3 && (
  ...
  return (
    <Panel>
      ...
      <p className="text-sm text-muted-foreground">
        Let's connect your channel and the wolfaid bot account, set a few defaults, and you're live.
      </p>
    ...
    <div className="flex items-center justify-between rounded-md border p-3 text-sm">
      ...
      {connected ? (
        ...
        <Button
          ...
          disabled={disabled}
          onClick={() => {
            window.location.href = `${env.VITE_SERVER_URL}/twitch/connect/${role}`;
          }}
        >
          Connect
        </Button>
    ...
    <Panel>
      ...
      {!data.twitchAppConfigured && (
        <p className="rounded-md border border-yellow-600/40 bg-yellow-500/10 p-3 text-xs text-yellow-600 dark:text-yellow-400">
          Set <code>TWITCH_CLIENT_ID</code> and <code>TWITCH_CLIENT_SECRET</code> in the server env
          (register an app at dev.twitch.tv/console/apps, redirect{" "}
          <code>{env.VITE_SERVER_URL}/twitch/callback</code>) to enable connecting. You can skip and
          do this later.
        </p>
      )}
      <ConnectRow
